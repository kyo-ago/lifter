export async function findCertificate(certificateName: string): Promise<string> {
    return ({
        "missing certificate": () => { throw new Error("security: SecKeychainSearchCopyNext: The specified item could not be found in the keychain.\n"); },
...
export function deleteCertificate(certificateName: string): Promise<string> {
    return ({
        "missing certificate": () => { throw new Error("Unable to delete certificate matching \"NodeMITMProxyCA\"\n"); },
        "": () => "",
...
export function importCert(certificatePath: string): Promise<string> {
    return ({
        ...
        "no such file or directory": () => { throw new Error("security: Error reading infile /unknown: No such file or directory\n"); },
