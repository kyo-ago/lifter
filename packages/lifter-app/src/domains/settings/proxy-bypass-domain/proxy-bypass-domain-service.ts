...
export class ProxyBypassDomainService {
    ...
    private async setProxyBypassDomains() {
        return await this.networksetupProxyService
            .getNetworksetupProxy()
            .map(async networksetupProxy => {
                ...
                let promises = allInterfaces.map(ni =>
                    ni.setProxyBypassDomains(networksetupProxy, proxyBypassDomainEntities)
                );
